...
# Fixed seeds per sample so re-runs (and cross-variant runs) produce
# byte-identical FASTQs. Diffing snakemake7/results vs snakemake9/results
# is a useful migration sanity check — that only works if the inputs match.
declare -A SEEDS=( [sampleA]=42 [sampleB]=137 )
...
for sample in sampleA sampleB; do
  ...
  if [[ ! -s "$fq" ]]; then
    ...
    wgsim -N 5000 -1 100 -2 100 -e 0.001 -r 0.001 -S "${SEEDS[$sample]}" \
